...
namespace Stataria
{
    public class GuardianAuraDrawLayer : PlayerDrawLayer
    {
        ...
        protected override void Draw(ref PlayerDrawSet drawInfo)
        {
            ...
            if (rpgPlayer?.ActiveRole?.ID != "Guardian" || rpgPlayer.ActiveRole.Status != RoleStatus.Active)
                return;

            var config = ModContent.GetInstance<StatariaConfig>();
            float radius = config.roleSettings.GuardianAuraRadius;

            Vector2 position = player.Center - Main.screenPosition;
            
            Color auraColor = Color.Gray * 0.2f;
            ...
            for (int angle = 0; angle < 360; angle += 2)
            {
                    ...
                    dustPos.Y > -50 && dustPos.Y < Main.screenHeight + 50)
                {
                    Dust dust = Dust.NewDustPerfect(dustPos + Main.screenPosition, 
                        DustID.Mercury, Vector2.Zero, 0, Color.Gray * 0.6f, 0.8f);
                    dust.noGravity = true;
                    dust.fadeIn = 0.3f;
            ...
            for (int i = 0; i < 20; i++)
            {
                ...
                if (Main.rand.NextBool(30))
                {
                    Dust innerDust = Dust.NewDustPerfect(innerPos + Main.screenPosition,
                        DustID.Mercury, Vector2.Zero, 0, Color.LightGray * 0.4f, 0.5f);
